...
#pragma comment(lib, "user32.lib") //setcursorpos 에러 잡기 위해 추가
...
int main(int argc, char** argv)
{
	...
	cout << "if you want to move cursor, make your finger Three!" << endl;
	...
	while (true)
	{
		...
		namedWindow("minmax");
		imshow("minmax", tmpImg);

		/*
		if (!contours.empty()) {
			vector<vector<Point>>hull(1);
			convexHull(Mat(contours[3]), hull[0]);
